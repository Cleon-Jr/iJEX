            ...
            while not(Eof) do
              begin
                  ...
                  if(FieldByName('tipo_cat_codigo').AsString = '1')then
                    begin
                      tipo_cat_codigo:= 'Profissionais do Magistério';
                    end;
                  if(FieldByName('tipo_cat_codigo').AsString = '2')then
                    begin
                      tipo_cat_codigo:= 'Outros Profissionais da Educação';
                    end;
                    ...
                    2:
                      situacao_siope := 'TEMPORÁRIO';
                    3:
                      situacao_siope := 'PROFISSIONAL DA EDUCAÇÃO EM ATIVIDADE ALHEIA À MDE';
                    4:
                      ...
                      begin
                        if(FieldByName('fundeb70_siope').AsString = 'N') and (FieldByName('fundeb30_siope').AsString = 'N') then
                          begin
                            ...
                            inttostr(ln) + '- Matrícula: ' + FieldByName('matricula').AsString + ' CPF: ' + FieldByName('cnpf').AsString +
                            ' - ' + tipo_cat_codigo + ' - Fundeb não definido: 30% ou 70%'
                            );
                          end;
                      end;
                      ...
                      begin
                        if(FieldByName('art61_phnms_siope').AsString = 'N') and
                          ...
                          begin
                            Memo1.Lines.Add('Linha ' +
                            inttostr(ln) + '- Matrícula: ' + FieldByName('matricula').AsString + ' CPF: ' + FieldByName('cnpf').AsString +
                            ' - Qualificação do FUNDEB não informada!'
                            );
                          end;
              ...
              Application.MessageBox('Arquivo gerado com sucesso! A pasta será aberta.',' iJEX - Geração de arquivo SIOPE | Sucesso', MB_ICONEXCLAMATION + MB_OK);
...
Application.MessageBox('Código de Eventos salvo com sucesso!',' iJEX | SUCESSO', MB_ICONEXCLAMATION + MB_OK);
      ...
      with(DM.sqlUpdates)do
        begin
          ...
          Except on E: Exception do
            begin
              Application.MessageBox(Pchar('Não foi possível corrigir o registro! ' + E.Message),' iJEX | ERRO', MB_ICONERROR + MB_OK);
              Abort;
            end;
    ...
    with(DM.sqlErrors)do
      begin
        ...
        if(RowsAffected > 0)then
          begin
            if(Application.MessageBox(Pchar(IntToStr(RecordCount) + ' inconsistência(s) encontrada(s): definição de 30% ou 70% do FUNDEB ausente.' + #13 +
            'Deseja que a aplicação corrija esses registros? Registros serão definidos para 70% do FUNDEB.'),' iJEX | ATENÇÃO', MB_ICONQUESTION + MB_YESNO) = IDYES)then
                begin
                  while not(Eof) do
                    begin
